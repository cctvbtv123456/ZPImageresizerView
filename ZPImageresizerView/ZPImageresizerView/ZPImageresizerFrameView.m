...
#import "ZPImageresizerFrameView.h"
...
@interface ZPImageresizerFrameView ()
...
@implementation ZPImageresizerFrameView
{
...
- (void)setStrokeColor:(UIColor *)strokeColor {
    ...
    if (_frameType == ZPConciseFrameType ||
        _frameType == ZPConciseWithoutOtherDotFrameType) {
...
- (void)setFrameType:(ZPImageresizerFrameType)frameType {
    ...
    CGFloat lineW = 0;
    if (frameType == ZPConciseFrameType ||
        frameType == ZPConciseWithoutOtherDotFrameType) {
        if (frameType == ZPConciseFrameType) {
            [self leftMidDot];
            [self rightMidDot];
            [self topMidDot];
...
- (CGSize)imageViewSzie {
    ...
    if (self.rotationDirection == ZPImageresizerVerticalUpDirection ||
        self.rotationDirection == ZPImageresizerVerticalDownDirection) {
        return CGSizeMake(w, h);
    } else {
...
- (BOOL)isHorizontalDirection {
    return (self.rotationDirection == ZPImageresizerHorizontalLeftDirection ||
            self.rotationDirection == ZPImageresizerHorizontalRightDirection);
}
...
- (instancetype)initWithFrame:(CGRect)frame
 ...
 imageresizerIsPrepareToScale:(ZPImageresizerIsPrepareToScaleBlock)imageresizerIsPrepareToScale {
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        _minImageWH = 70.0;
        _sizeScale = 1.0;
        _rotationDirection = ZPImageresizerVerticalUpDirection;
...
- (BOOL)isShowMidDot {
    return _isArbitrarily && _frameType == ZPConciseFrameType;
}
...
- (BOOL)imageresizerFrameIsEqualImageViewFrame {
    CGSize imageresizerSize = self.imageresizerSize;
    CGSize imageViewSzie = self.imageViewSzie;
    CGFloat resizeWHScale = (self.rotationDirection == ZPImageresizerVerticalUpDirection || self.rotationDirection == ZPImageresizerVerticalDownDirection) ? _resizeWHScale : (1.0 / _resizeWHScale);
...
- (void)hideOrShowBlurEffect:(BOOL)isHide animateDuration:(NSTimeInterval)duration {
    if (self.maskType == ZPNormalMaskType) return;
...
- (void)hideOrShowFrameLine:(BOOL)isHide animateDuration:(NSTimeInterval)duration {
    if (self.frameType != ZPClassicFrameType) return;
...
- (void)updateImageresizerFrame:(CGRect)imageresizerFrame animateDuration:(NSTimeInterval)duration {
    ...
    if (_frameType == ZPConciseFrameType || _frameType == ZPConciseWithoutOtherDotFrameType) {
    ...
    if (duration > 0) {
        ...
        } else if (_frameType == ZPClassicFrameType) {
...
- (void)updateImageOriginFrameWithDirection:(ZPImageresizerRotationDirection)rotationDirection {
    [self removeTimer];
...
- (void)updateRotationDirection:(ZPImageresizerRotationDirection)rotationDirection {
    [self updateMaxResizeFrameWithDirection:rotationDirection];
    if (!_isArbitrarily) {
        BOOL isVer2Hor = ((_rotationDirection == ZPImageresizerVerticalUpDirection ||
                           _rotationDirection == ZPImageresizerVerticalDownDirection) &&
                          (rotationDirection == ZPImageresizerHorizontalLeftDirection ||
                           rotationDirection == ZPImageresizerHorizontalRightDirection));
        BOOL isHor2Ver = ((_rotationDirection == ZPImageresizerHorizontalLeftDirection ||
                           _rotationDirection == ZPImageresizerHorizontalRightDirection) &&
                          (rotationDirection == ZPImageresizerVerticalUpDirection ||
                           rotationDirection == ZPImageresizerVerticalDownDirection));
        if (isVer2Hor || isHor2Ver) _resizeWHScale = 1.0 / _resizeWHScale;
    }
    _rotationDirection = rotationDirection;
}
    
- (void)updateMaxResizeFrameWithDirection:(ZPImageresizerRotationDirection)direction {
    CGFloat x = 0;
    CGFloat y = 0;
    CGFloat w = 0;
    CGFloat h = 0;
    if (direction == ZPImageresizerVerticalUpDirection ||
        direction == ZPImageresizerVerticalDownDirection) {
    ...
    CGFloat lineW = 0;
    if (_frameType == ZPClassicFrameType) lineW = _arrLineW / _sizeScale;
...
- (void)checkIsCanRecovery {
    ...
    BOOL isOriginFrame = (self.rotationDirection == ZPImageresizerVerticalUpDirection &&
...
- (void)updateImageresizerFrameWithVerBaseMargin:(CGFloat)verBaseMargin horBaseMargin:(CGFloat)horBaseMargin {
    ...
    [self updateImageOriginFrameWithDirection:ZPImageresizerVerticalUpDirection];
}
...
- (void)recoveryWithDuration:(NSTimeInterval)duration {
    [self updateRotationDirection:ZPImageresizerVerticalUpDirection];
...
- (void)imageresizerWithComplete:(void (^)(UIImage *))complete isOriginImageSize:(BOOL)isOriginImageSize referenceWidth:(CGFloat)referenceWidth {
    ...
    switch (self.rotationDirection) {
        case ZPImageresizerHorizontalLeftDirection:
        orientation = UIImageOrientationLeft;
        break;
        
        case ZPImageresizerVerticalDownDirection:
        orientation = UIImageOrientationDown;
        break;
        
        case ZPImageresizerHorizontalRightDirection:
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        image = [image zp_fixOrientation];
        if (isVerticalityMirror) image = [image zp_verticalityMirror];
        if (isHorizontalMirror) image = [image zp_horizontalMirror];
        ...
        resizeImg = [resizeImg zp_rotate:orientation];
