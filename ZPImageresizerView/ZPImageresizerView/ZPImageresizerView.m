...
@interface ZPImageresizerView () <UIScrollViewDelegate>
@property (nonatomic, weak) UIScrollView *scrollView;
@property (nonatomic, weak) UIImageView *imageView;
@property (nonatomic, weak) ZPImageresizerFrameView *frameView;

@property (nonatomic, strong) NSMutableArray *allDirections;
@property (nonatomic, assign) NSInteger directionIndex;
@end

@implementation ZPImageresizerView
    {
...
- (ZPImageresizerMaskType)maskType {
    return self.frameView.maskType;
}
    
- (ZPImageresizerFrameType)frameType {
...
+ (instancetype)imageresizerViewWithConfigure:(ZPImageresizerConfigure *)configure
    ...
    ZPImageresizerView *imageresizerView =  [[self alloc] initWithResizeImage:configure.resizeImage
...
- (void)setupBase {
    ...
    self.allDirections = [@[@(ZPImageresizerVerticalUpDirection),
                            @(ZPImageresizerHorizontalLeftDirection),
                            @(ZPImageresizerVerticalDownDirection),
                            @(ZPImageresizerHorizontalRightDirection)] mutableCopy];
}
...
- (void)setupFrameViewWithMaskType:(ZPImageresizerMaskType)maskType
                         frameType:(ZPImageresizerFrameType)frameType
                    animationCurve:(ZPAnimationCurve)animationCurve
                       strokeColor:(UIColor *)strokeColor
                         maskAlpha:(CGFloat)maskAlpha
                     resizeWHScale:(CGFloat)resizeWHScale
                isCanRecoveryBlock:(ZPImageresizerIsCanRecoveryBlock)isCanRecoveryBlock
             isPrepareToScaleBlock:(ZPImageresizerIsPrepareToScaleBlock)isPrepareToScaleBlock {
    
    ZPImageresizerFrameView *frameView =
    [[ZPImageresizerFrameView alloc] initWithFrame:self.scrollView.frame
...
- (void)rotation {
    ...
    ZPImageresizerRotationDirection direction = [self.allDirections[self.directionIndex] integerValue];
    
    CGFloat scale = 1;
    if (self.isRotatedAutoScale) {
        if (direction == ZPImageresizerHorizontalLeftDirection ||
            direction == ZPImageresizerHorizontalRightDirection) {
